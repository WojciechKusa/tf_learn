...
def visualize(embeddings, iteration):
  try:
    ...
    filename = 'embeddings-iter' + str(iteration) + '.png'
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    if step % 10000 == 0:
      ...
      visualize(final_embeddings, step)


plt.clf()
N = 20
loss_array = np.convolve(loss_array, np.ones((N,))/N, mode='valid')
plt.plot(loss_array)
# plt.plot(sm, 'r')
